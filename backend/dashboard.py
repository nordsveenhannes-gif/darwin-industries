...
<style>
:root{--bg:#090d13;--panel:#111925;--line:#243247;--text:#edf5ff;--muted:#91a4bc;--good:#44e39b;--blue:#70b7ff;--warn:#ffd166;--danger:#ff7b7b}
*{box-sizing:border-box}body{margin:0;background:#090d13;color:var(--text);font:14px/1.45 Segoe UI,Arial,sans-serif}header{position:sticky;top:0;background:#0b111a;border-bottom:1px solid var(--line);padding:15px 22px;display:flex;justify-content:space-between}.brand{font-size:20px;font-weight:800}.owner{color:var(--good);font-weight:700}main{max-width:1500px;margin:auto;padding:20px}.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:16px;margin-bottom:16px}h2{font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#b9cae0}.metrics,.agents{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}.metric,.agent{background:#0d141e;border:1px solid var(--line);border-radius:12px;padding:12px}.agent{position:relative;overflow:hidden;transition:.2s transform,.2s border-color,.2s box-shadow}.agent:hover{transform:translateY(-2px)}.agent.working{border-color:var(--good);box-shadow:0 0 0 1px rgba(68,227,155,.18),0 0 28px rgba(68,227,155,.10)}.agent.working:before{content:"";position:absolute;inset:0 auto 0 0;width:3px;background:var(--good)}.agent.waiting{border-color:var(--warn)}.agent.off{opacity:.58}.agent-avatar{width:52px;height:52px;border-radius:16px;display:grid;place-items:center;font-size:28px;background:linear-gradient(145deg,#182334,#0b111a);border:1px solid #2c4058;box-shadow:inset 0 1px rgba(255,255,255,.05)}.agent-top{display:flex;align-items:center;gap:12px}.agent-meta{min-width:0}.working-badge{display:inline-flex;align-items:center;gap:6px;font-size:10px;font-weight:900;letter-spacing:.08em;color:var(--good)}.working-badge:before{content:"";width:7px;height:7px;border-radius:50%;background:currentColor;box-shadow:0 0 12px currentColor}.agent-quips{color:#73869d;font-size:10px;margin-top:8px;font-style:italic}.now-label{font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#6f859f;margin-top:10px}.action{font-size:12px;margin-top:3px}.active-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.active-card{background:#0c151f;border:1px solid #26415a;border-radius:12px;padding:12px;display:flex;gap:12px;align-items:center}.active-card strong{display:block}.active-card .small{margin-top:3px}.metric b{display:block;font-size:22px}.metric span,.small{color:var(--muted);font-size:11px}.name{font-weight:800}.role{color:var(--muted);font-size:11px}.status{display:inline-block;margin:7px 0;padding:3px 7px;border-radius:20px;background:#1d2a3b;color:var(--blue);font-size:10px}.action{font-size:12px}table{width:100%;border-collapse:collapse;font-size:12px}td,th{padding:8px;border-bottom:1px solid #203047;text-align:left}th{color:var(--muted)}.good{color:var(--good)}@media(max-width:900px){.metrics,.agents,.active-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:560px){.active-grid{grid-template-columns:1fr}}
</style></head><body>
...
async function refresh(){
 try{
  ...
  const working=d.agents.filter(a=>stateClass(a.status)==='working'||stateClass(a.status)==='waiting');
  document.getElementById('activeAgents').innerHTML=working.length?working.map(a=>{const p=persona[a.agent]||['👤',''];return '<div class="active-card"><div class="agent-avatar">'+p[0]+'</div><div><strong>'+esc(a.agent)+' · '+esc(a.status)+'</strong><div class="small">'+esc(a.last_action||'Nothing assigned')+'</div></div></div>'}).join(''):'<div class="small">Nobody is actively working right now. Either the company is sleeping or something has gone suspiciously well.</div>';
  ...
  const sg=d.trade_signals.map(x=>'<tr><td>'+esc(x.agent)+'</td><td>'+esc(x.asset_class)+'</td><td>'+esc(x.symbol)+'</td><td>'+esc(x.action)+'</td><td>'+esc(x.confidence)+'</td><td>'+esc(x.thesis||'')+'</td><td>'+esc((x.created_at||'').replace('T',' ').slice(0,19))+'</td></tr>').join('');
  document.getElementById('signals').innerHTML='<h2 style="margin-top:18px">Latest signals</h2><table><thead><tr><th>Agent</th><th>Class</th><th>Symbol</th><th>Action</th><th>Confidence</th><th>Why</th><th>Time</th></tr></thead><tbody>'+sg+'</tbody></table>';
...
def state_payload():
    ...
    signals=[dict(r) for r in conn.execute("SELECT agent,asset_class,symbol,action,confidence,thesis,risk_decision,created_at FROM trade_signals ORDER BY id DESC LIMIT 20").fetchall()]
...
+Number(wp.quoted_price||0).toFixed(0)+' • care <h2 style="margin-top:18px">Client brief / clarifications</h2><table><thead><tr><th>Gate</th><th>Question</th><th>Status</th><th>Answer</th></tr></thead><tbody>'+cq+'</tbody></table><h2 style="margin-top:18px">Project events</h2><table><thead><tr><th>Agent</th><th>Stage</th><th>Detail</th></tr></thead><tbody>'+ev+'</tbody></table><h2 style="margin-top:18px">Staging enquiries</h2><table><thead><tr><th>Name</th><th>Email</th><th>Interest</th><th>Status</th></tr></thead><tbody>'+wl+'</tbody></table>';
  }else{
    document.getElementById('website').innerHTML='<div class="small">No website project yet.</div>';
  }
  document.getElementById('stats').innerHTML=d.trader_stats.map(x=>{const pf=x.profit_factor==null?'∞':Number(x.profit_factor).toFixed(2);return '<div class="metric"><span>'+esc(x.agent)+' '+esc(x.asset_class)+'</span><b>$'+Number(x.net_pnl||0).toFixed(2)+'</b><div class="small">closed '+esc(x.closed_trades)+' • win '+Number(x.win_rate||0).toFixed(1)+'% • PF '+esc(pf)+' • drawdown $'+Number(x.max_drawdown||0).toFixed(2)+' • '+esc(x.paper_verdict)+'</div></div>'}).join('');
...
if __name__ == '__main__': main()
+Number(wp.monthly_price||0).toFixed(0)+'/mo • revisions '+esc(wp.revision_rounds_used||0)+'/2 • approval '+esc(wp.customer_approval_status||'PENDING')+' • brief '+esc(d.website_client_answered||0)+'/'+esc(d.website_client_total||0)+' • client images '+esc(d.website_asset_count||0)+' • delivery email '+esc(wp.delivery_email_status||'NOT SENT')+' • '+preview+' • leads '+esc(d.website_lead_count||0)+'</div></div><h2 style="margin-top:18px">Client brief / clarifications</h2><table><thead><tr><th>Gate</th><th>Question</th><th>Status</th><th>Answer</th></tr></thead><tbody>'+cq+'</tbody></table><h2 style="margin-top:18px">Project events</h2><table><thead><tr><th>Agent</th><th>Stage</th><th>Detail</th></tr></thead><tbody>'+ev+'</tbody></table><h2 style="margin-top:18px">Staging enquiries</h2><table><thead><tr><th>Name</th><th>Email</th><th>Interest</th><th>Status</th></tr></thead><tbody>'+wl+'</tbody></table>';
  }else{
    document.getElementById('website').innerHTML='<div class="small">No website project yet.</div>';
  }
  document.getElementById('stats').innerHTML=d.trader_stats.map(x=>{const pf=x.profit_factor==null?'∞':Number(x.profit_factor).toFixed(2);return '<div class="metric"><span>'+esc(x.agent)+' '+esc(x.asset_class)+'</span><b>$'+Number(x.net_pnl||0).toFixed(2)+'</b><div class="small">closed '+esc(x.closed_trades)+' • win '+Number(x.win_rate||0).toFixed(1)+'% • PF '+esc(pf)+' • drawdown $'+Number(x.max_drawdown||0).toFixed(2)+' • '+esc(x.paper_verdict)+'</div></div>'}).join('');
