...
async function refresh(){
 try{
  ...
  document.getElementById('stats').innerHTML=d.trader_stats.map(function(x){
    ...
    return '<div class="metric"><span>'+esc(x.agent)+' '+esc(x.asset_class)+'</span><b>
  const tr=d.trades.map(function(t){
    ...
    return '<tr><td>'+esc(t.asset_class)+'</td><td>'+esc(t.symbol)+'</td><td>'+esc(t.trade_mode||'-')+'</td><td>'+esc(t.setup_score==null?'-':t.setup_score)+'</td><td>'+esc(t.status)+'</td><td>
...
if __name__ == "__main__":
    main()
+Number(x.net_pnl||0).toFixed(2)+'</b><div class="small">closed '+esc(x.closed_trades)+' • win '+Number(x.win_rate||0).toFixed(1)+'% • PF '+esc(pf)+' • expectancy '+Number(x.expectancy_r||0).toFixed(2)+'R • drawdown 
  const tr=d.trades.map(function(t){return '<tr><td>'+esc(t.asset_class)+'</td><td>'+esc(t.symbol)+'</td><td>'+esc(t.status)+'</td><td>$'+Number(t.notional_usd||0).toFixed(2)+'</td><td>'+Number(t.entry_price||0).toFixed(6)+'</td><td>'+(t.exit_price==null?'-':Number(t.exit_price).toFixed(6))+'</td><td>$'+Number(t.fees_usd||0).toFixed(2)+'</td><td>$'+Number(t.pnl_usd||0).toFixed(2)+'</td></tr>'}).join('');
  document.getElementById('trades').innerHTML='<h2 style="margin-top:18px">Paper trades</h2><div class="scroll"><table><thead><tr><th>Class</th><th>Symbol</th><th>Status</th><th>Notional</th><th>Entry</th><th>Exit</th><th>Fees</th><th>Net P&L</th></tr></thead><tbody>'+tr+'</tbody></table></div>';

  const sg=d.trade_signals.map(function(x){return '<tr><td>'+esc(x.agent)+'</td><td>'+esc(x.asset_class)+'</td><td>'+esc(x.symbol)+'</td><td>'+esc(x.action)+'</td><td>'+esc(x.confidence)+'</td><td>'+esc(x.thesis||'')+'</td><td>'+esc((x.created_at||'').replace('T',' ').slice(0,19))+'</td></tr>'}).join('');
  document.getElementById('signals').innerHTML='<h2 style="margin-top:18px">Latest signals</h2><div class="scroll"><table><thead><tr><th>Agent</th><th>Class</th><th>Symbol</th><th>Action</th><th>Confidence</th><th>Why</th><th>Time</th></tr></thead><tbody>'+sg+'</tbody></table></div>';
...
def state_payload():
    ...
    trades = [
        dict(r)
        for r in conn.execute(
            """SELECT asset_class,symbol,status,notional_usd,entry_price,exit_price,fees_usd,pnl_usd,opened_at,closed_at
            FROM paper_trades ORDER BY id DESC LIMIT 20"""
        ).fetchall()
    ]
    signals = [
...
if __name__ == "__main__":
    main()
+Number(x.max_drawdown||0).toFixed(2)+' • '+esc(x.paper_verdict)+'</div></div>';
  }).join('');
...
if __name__ == "__main__":
    main()
+Number(t.notional_usd||0).toFixed(2)+'</td><td>'+r+'</td><td>
  const pr=d.prospects.map(function(p){return '<tr><td>'+esc(p.business_name)+'</td><td>'+esc(p.city)+'</td><td>'+esc(p.category)+'</td><td>'+esc(p.sales_score==null?'-':p.sales_score)+'</td><td>'+esc(p.status)+'</td></tr>'}).join('');
  document.getElementById('pipeline').innerHTML='<table><thead><tr><th>Business</th><th>Market</th><th>Category</th><th>Score</th><th>Status</th></tr></thead><tbody>'+pr+'</tbody></table>';
 }catch(e){
  document.getElementById('metrics').innerHTML='<div class="small">Refresh error: '+esc(e)+'</div>';
...
if __name__ == "__main__":
    main()
+Number(t.pnl_usd||0).toFixed(2)+'</td></tr>';
  }).join('');
